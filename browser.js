export const isPhone:boolean = checkMobile();
...
function collapseOnPhone() {
	if (isPhone) {
		...
		while (imgs.length > 0) {
			...
			let par = img.parentNode as Element;
			//console.log(img.src+" @ "+par.tagName);
			if (par.tagName == "VIDEO") {
				let put = document.createElement("div") as HTMLDivElement;
				put.innerHTML = '<a href = "'+img.src+'">'+img.src+"</a>";
				(par.parentNode as Element).replaceChild(put, par);
			}
			else {
		...
		for (var i = 0; i < text.length; i++) {
			(text[i] as HTMLDivElement).style.width = "33%";
		}
		let header = document.createElement("div") as HTMLDivElement;
		...
		(navbar.parentNode as Element).insertBefore(header, navbar.nextSibling);
		
		let toc = document.getElementById("toc_container") as HTMLDivElement;
		(toc.parentNode as Element).removeChild(toc);
	}
}
